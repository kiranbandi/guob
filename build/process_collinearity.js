...
function pullGeneInfo(){
    ...
    d3.text("at_vv_collinear.collinearity").then(function (data){

        let rawCollinearity = process(data);
        
        let selectedCollinearity = rawCollinearity.alignmentList;
...
d3.text('at_vv_coordinate.gff').
then(function(d){
    var result = "chromosome\tgene\tstart\tend\n" + d;
    window.dataset = d3.tsvParse(result)
