...
import {handleZoomCanvas, handleZoomSVG} from './actions'
...
export function draw(arrayCoordinates, ctx,curvature){
    ...
    arrayCoordinates.map((item, index) => {
        var p = new Path2D(createLinkLinePath({ item }, curvature));
        ...
        if (item.type === "line") {
            ...
            var p = new Path2D(createLinkPolygonPath({ item }, curvature));
            ...
            if (color) {
                ...
                ctx.strokeStyle = 'none';
                ctx.fillStyle = Turbocolors[Math.round((index + 1) * Turbocolors.length / (totalLines + 1))]
...
const Links = ({ arrayCoordinates, curvature = 0.5, type = "svg" }) => {
    ...
    if (type.toLowerCase() === "svg") {
        ...
        pathList = arrayCoordinates.map((item, index) => {
            if (item.type === "line") {
                var color = item.color;
                if (color) { return <path d={createLinkLinePath({ item }, curvature)} key={index} fill="none" stroke={color} strokeWidth="2px"></path> }
                else {
        ...
        return (<svg ref={svgRef} width={950} height={100} className="LinksSVG" onClick={()=>dispatch(handleZoomSVG(svgGRef,svgRef))}>
        ...
        return (<canvas ref={canvasRef} width={950} height={100} onClick={()=>dispatch(handleZoomCanvas(canvasRef, canvasRef, arrayCoordinates, curvature))
        }/>)
    }
