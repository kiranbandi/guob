...
const EventEmitter = __webpack_require__(/*! ./EventEmitter */ "./src/EventEmitter.js")
...
class GT extends EventEmitter {
  ...
  constructor (serverIp, opts = {}) {
    ...
    socket.on('connected', (id, user) => {
      ...
      console.log(`ID ${id} has joined with state:`, user);
  ...
  $( "#draggable" ).draggable();
    ...
    socket.on('authed', (authPayload) => {
      // update our state variables.
      console.log('Authed', authPayload.id, 'with state', authPayload.state)
  ...
  /**
     ...
     * @param {Object} userPayload An optional initial state we have as a user.
     */
  join (room, userPayload={color:colorHash}) {
    return new Promise((resolve, reject) => {
      if (!this.isConnected()) { reject(new Error('We need to be connected.')) }
...
const GT = __webpack_require__(/*! ./GT */ "./src/GT.js")
